...
export const fetchHomeFeed = async (req,res)=>{
    try {
        const events = await Event.find({ moderationStatus: "APPROVED" }).select('name shortDescription image adminId category eventDate eventStartTime eventEndTime venue club')
